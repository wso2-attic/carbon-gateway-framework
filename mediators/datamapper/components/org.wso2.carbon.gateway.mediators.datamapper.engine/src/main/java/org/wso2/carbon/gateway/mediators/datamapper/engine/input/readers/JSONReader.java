...
import org.wso2.carbon.gateway.mediators.datamapper.engine.core.exceptions.SimpleJSONParserException;
...
import static org.wso2.carbon.gateway.mediators.datamapper.engine.utils.DataMapperEngineConstants.ARRAY_ELEMENT_TYPE;
...
import static org.wso2.carbon.gateway.mediators.datamapper.engine.utils.DataMapperEngineConstants.NULL_ELEMENT_TYPE;
import static org.wso2.carbon.gateway.mediators.datamapper.engine.utils.DataMapperEngineConstants.NUMBER_ELEMENT_TYPE;
import static org.wso2.carbon.gateway.mediators.datamapper.engine.utils.DataMapperEngineConstants.OBJECT_ELEMENT_TYPE;
...
public class JSONReader implements Reader, ContentHandler {
    ...
    @Override public void startJSON() throws ParseException, IOException {
    }

    @Override public void endJSON() throws ParseException, IOException {
        try {
            sendTerminateEvent();
    ...
    @Override public boolean startObject() throws ParseException, IOException {
        try {
    ...
    @Override public boolean startObjectEntry(String s) throws ParseException, IOException {
        try {
    ...
    @Override public boolean startArray() throws ParseException, IOException {
        try {
            if (!getReaderEventStack().isEmpty()) {
                ReaderEvent stackElement = getReaderEventStack().peek();
                String type = getInputSchema().getElementTypeByName(schemaElementList);
    ...
    @Override public boolean primitive(Object value) throws ParseException, IOException {
        if (!getReaderEventStack().isEmpty()) {
            ReaderEvent stackElement = getReaderEventStack().peek();
